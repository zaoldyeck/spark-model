...
/**
  ...
  */
class ALSPredictPlay extends ALSFold {
  ...
  override def run(implicit sc: SparkContext): Unit = {
    ...
    Random.shuffle(parametersSeq).zipWithIndex foreach {
      case (parameters, index) =>
        ...
        try {
          val model: MatrixFactorizationModel = ALS.trainImplicit(targetGameTraining union notTargetTraining, parameters.rank, 50, parameters.lambda, parameters.alpha)
    ...
    dropHeader(data) flatMap {
      _.split(",") match {
        case Array(unique_id, game_index, saving, max_login_days, max_login_times, game_nums) =>
          ...
          Some(Rating(unique_id.toInt, game_index.toInt, if (rating >= 3) 1 else 0))
        case some =>
