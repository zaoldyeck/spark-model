...
/**
  ...
  */
class ALSPredictPlay(loginDay: Int) extends ALSFold {
  private val DataPath: String = "s3n://data.emr/web_user_game_nums.csv"
  private val OutputPath: String = s"/home/hadoop/output/als-play-login$loginDay.csv"
    ...
    val futures: IndexedSeq[Future[Unit]] = Random.shuffle(parametersSeq).zipWithIndex map {
      case (parameters, index) =>
        Future {
          ...
          try {
            val model: MatrixFactorizationModel = ALS.trainImplicit(targetGameTraining union notTargetTraining, parameters.rank, 100, parameters.lambda, parameters.alpha)
          ...
          */
            ...
            val delete_out_path1: String = "hadoop fs -rm -f -r ./als-play-78"
            delete_out_path1.!

            val delete_out_path2: String = "hadoop fs -rm -f -r ./als-play-not-78"
            delete_out_path2.!

            predictResult1.saveAsTextFile("./als-play-78")
            predictResult2.saveAsTextFile("./als-play-not-78")
  ...
  override def mappingData(data: RDD[String]): RDD[Rating] = {
    ...
    dropHeader(data) flatMap {
      _.split(",") match {
        case Array(unique_id, game_index, saving, max_login_days, max_login_times, game_nums) =>
          val rating: Double = max_login_days.toDouble
          Some(Rating(unique_id.toInt, game_index.toInt, if (rating >= loginDay) 1 else 0))
        case some =>
