...
class TestALS {
  ...
  def preparation(rawUserArtistData: RDD[String], rawArtistData: RDD[String], rawArtistAlias: RDD[String]): Unit = {
    ...
    Logger.log.warn(userIDStats)
    Logger.log.warn(itemIDStats)

    val artistByID = buildArtistByID(rawArtistData)
    val artistAlias = buildArtistAlias(rawArtistAlias)

    val (badID, goodID) = artistAlias.head
    Logger.log.warn(artistByID.lookup(badID) + " -> " + artistByID.lookup(goodID))
  }

  def buildRatings(rawUserArtistData: RDD[String], bArtistAlias: Broadcast[Map[Int, Int]]): RDD[Rating] = {
  ...
  def model(sc: SparkContext, rawUserArtistData: RDD[String], rawArtistData: RDD[String], rawArtistAlias: RDD[String]): Unit = {
    ...
    trainData.unpersist()
    Logger.log.warn(model.userFeatures.mapValues(_.mkString(", ")).first())

    val userID = 2093760
    val recommendations = model.recommendProducts(userID, 5)
    recommendations.foreach(Logger.log.warn)
    ...
    }.values.collect().foreach(Logger.log.warn)
  ...
  def evaluate(sc: SparkContext, rawUserArtistData: RDD[String], rawArtistAlias: RDD[String]): Unit = {
    ...
    Logger.log.warn(mostListenedAUC)
    ...
    evaluations.sortBy(_._2).reverse.foreach(Logger.log.warn)
    trainData.unpersist()
    cvData.unpersist()
  }
